...
impl MqttPublisher {
    pub fn new(
        ...
        let mut mqttoptions = MqttOptions::new(&mqtt_config.client_id, &mqtt_config.host, mqtt_config.port);
        mqttoptions.set_keep_alive(Duration::from_secs(20));
        mqttoptions.set_clean_session(true);

        if let (Some(ref username), Some(ref password)) = (&mqtt_config.username, &mqtt_config.password) {
    ...
    pub fn publish_discovery<'a>(&self, meter_config: &MeterConfig) -> Result<(), MqttError> {
        let device_info = self.create_device_info(meter_config);
        let state_topic = format!(
            "{}/sensor/{}/state",
            self.ha_config.discovery_prefix, self.ha_config.node_id
        ); 
        let unique_id = | object_id: &str | {
            format!("water_meter_{}_{}", meter_config.serial, object_id)
        };
