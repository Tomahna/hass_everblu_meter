...
fn run() -> Result<(), Box<dyn std::error::Error>> {
    ...
    info!("Reading meter serial={} year={}", config.meter.serial, config.meter.year);
    let meter_data = cc1101.get_meter_data(config.meter.year, config.meter.serial)?;
    info!("Meter data read successfully:\n{:?}", meter_data);
    info!("Publishing sensor to mqtt broker {}:{}", config.mqtt.host, config.mqtt.port);
...
fn publish_to_mqtt(config: &Config, meter_data: &MeterData) -> Result<(), Box<dyn std::error::Error>> {
