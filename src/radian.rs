	...
	for i in 0..l_total_byte {
		...
		for _j in 0..8 {
			if (current_rx_byte & 0x80) == bit_pol {
				...
				bit_cnt = (bit_cnt + 2) / 4;
				bit_cnt_flush_s8 = bit_cnt_flush_s8 - (bit_cnt as i8 * 4);

				for _k in 0..bit_cnt {
					// insert the number of decoded bit
					if dest_bit_cnt < 8 {
						// if data byte
						decoded[dest_byte_cnt as usize] = decoded[dest_byte_cnt as usize] >> 1;
                        decoded[dest_byte_cnt as usize] |= bit_pol;
					}
					dest_bit_cnt += 1;
					...
					if dest_bit_cnt >= 11 && bit_pol == 0 {  // start bit
						dest_bit_cnt = 0;
                        debug!("dec[{}]={} {}", i, dest_byte_cnt, decoded[dest_byte_cnt as usize]);
						dest_byte_cnt += 1;
					}
				}
...
pub fn make_radian_master_req(year: u8, serial: u32) -> [u8; 39] { 
    let mut to_encode: [u8; 19] = [
        0x13, 0x10, 0x00, 0x45, 0xFF, 0xFF, 0xFF, 0xFF,
        0x00, 0x45, 0x20, 0x0A, 0x50, 0x14, 0x00, 0x0A,
        0x40, 0xFF, 0xFF
    ];
    ...
    // Calculate CRC
    let crc = crc::Crc::<u16>::new(&crc::CRC_16_KERMIT).checksum(&to_encode[0..to_encode.len() - 2]);
...
mod tests {
    ...
    #[test]
    fn encode_works(){
        let input_buffer: [u8; 19] = [0x13, 0x10, 0x00, 0x45, 0x10, 0x10, 0x97, 0x8c, 0x00, 0x45, 0x20, 0x0a, 0x50, 0x14, 0x00, 0x0a, 0x40, 0xf1, 0xe3];
    ...
    fn make_radian_master_req_works(){
        assert_eq!(hex::encode(make_radian_master_req(16, 1087372)), "5000000003ffffffff64704700751704704774f18f00751702728705714700728701747f63ffff"); 
    }
}
